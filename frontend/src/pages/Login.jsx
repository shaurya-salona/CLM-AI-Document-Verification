...
const FormInput = ({ 
  ...
  icon: Icon, 
  placeholder,
  focusRingColor = 'ring-sky-500 border-sky-500'
}) => {
...
const Login = () => {
  ...
  const handleVendorSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role === 'vendor') {
        ...
        setVendorError('Account registered as Approver. Please use the TSL Approver Portal card on the right.');
      }
    } catch (err) {
  ...
  const handleApproverSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role === 'approver') {
        ...
        setApproverError('Account registered as Vendor. Please use the Vendor Portal card on the left.');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between relative overflow-hidden font-sans antialiased selection:bg-sky-500 selection:text-white">
      ...
      <div className="absolute bottom-1/4 right-1/4 translate-x-1/2 translate-y-1/2 w-[600px] h-[600px] bg-emerald-600/10 rounded-full blur-[120px] pointer-events-none animate-pulse duration-1000"></div>
      ...
      <header className="relative z-20 pt-6 px-6 max-w-7xl mx-auto w-full flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-900 pb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-center">
          ...
          <EnterpriseBadge icon={Sparkles} label="AI Verification" />
          <EnterpriseBadge icon={Award} label="ISO Compliant" />
        </div>
      </header>

      {/* Main Content Area */}
      <main className="relative z-10 my-auto py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10 space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold">
            <Sparkles className="w-3.5 h-3.5" /> AI Powered Vendor Compliance Verification
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
            Contract Labor Management System
          </h1>
          <p className="text-sm text-slate-400">
            Select your assigned portal access card below to authenticate and enter your workspace.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch max-w-5xl mx-auto">
          ...
          {/* =================================================================
             CARD 1: VENDOR PARTNER PORTAL (BLUE GRADIENT THEME)
             ================================================================= */}
          <div className="group relative bg-slate-900/60 backdrop-blur-xl border border-white/[0.08] hover:border-sky-500/40 rounded-3xl p-8 shadow-2xl transition-all duration-300 flex flex-col justify-between hover:shadow-sky-500/10">
            
            {/* Subtle Gradient Accent Border Line Top */}
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 mb-6 leading-relaxed">
                Submit company profile details, statutory numbers, and upload mandatory compliance PDFs.
              </p>
            ...
            <div className="mt-8 pt-4 border-t border-white/[0.06] flex items-center justify-between text-xs text-slate-400">
              ...
              <Link to="/register?role=vendor" className="text-sky-400 font-semibold hover:text-sky-300 hover:underline flex items-center gap-1 transition-colors">
          ...
          {/* =================================================================
             CARD 2: TSL APPROVER PORTAL (EMERALD GRADIENT THEME)
             ================================================================= */}
          <div className="group relative bg-slate-900/60 backdrop-blur-xl border border-white/[0.08] hover:border-emerald-500/40 rounded-3xl p-8 shadow-2xl transition-all duration-300 flex flex-col justify-between hover:shadow-emerald-500/10">
            
            {/* Subtle Gradient Accent Border Line Top */}
            <div className="absolute top-0 inset-x-8 h-px bg-gradient-to-r from-transparent via-emerald-500/50 to-transparent"></div>

            <div>
              {/* Card Title & Icon Header */}
              <div className="flex items-center justify-between mb-6">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-400 flex items-center justify-center shadow-lg shadow-emerald-600/30 group-hover:scale-105 transition-transform duration-300">
                ...
                <span className="text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  TSL Approver Workstation
                </span>
              </div>

              <h2 className="text-2xl font-bold text-white tracking-tight">
                TSL Approver Portal Login
              </h2>
              <p className="text-xs text-slate-400 mt-1 mb-6 leading-relaxed">
                Review site location vendor submissions, inspect OCR text layer, and evaluate AI verification remarks.
              </p>
              ...
              <form onSubmit={handleApproverSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between text-xs pt-1">
                  <label className="flex items-center gap-2 text-slate-400 hover:text-slate-200 cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={approverRemember}
                      onChange={(e) => setApproverRemember(e.target.checked)}
                      className="w-4 h-4 rounded bg-slate-950 border-slate-800 text-emerald-500 focus:ring-emerald-500/20 focus:ring-offset-0 cursor-pointer"
                    />
                    <span>Remember me</span>
                  </label>
                  <a href="#forgot" onClick={(e) => { e.preventDefault(); alert("Please contact Tata Steel Security Administrator to reset approver credentials."); }} className="text-emerald-400 font-medium hover:underline transition-colors">
                    Forgot password?
                  </a>
                </div>
                ...
                <div className="pt-2">
                  <PrimaryButton 
                    ...
                    gradient="from-emerald-600 via-teal-600 to-emerald-500 hover:from-emerald-500 hover:to-teal-400 shadow-emerald-500/25"
                  >
                    Sign In to TSL Portal <ArrowRight className="w-4 h-4" />
                  </PrimaryButton>
                </div>
              </form>
            </div>

            {/* Approver Card Footer with Clear Location-Based Approval Badges */}
            <div className="mt-8 pt-4 border-t border-white/[0.06] flex flex-col gap-1.5 text-xs">
              <div className="flex items-center justify-between text-slate-400">
                <span className="text-emerald-400 font-medium flex items-center gap-1">
                  <ShieldCheck className="w-3.5 h-3.5" /> Plant Location Site Approvers
                </span>
                <Link to="/register?role=approver" className="text-emerald-400 font-semibold hover:text-emerald-300 hover:underline flex items-center gap-1 transition-colors">
                  <UserPlus className="w-3.5 h-3.5" /> Register Approver
                </Link>
              </div>
              <div className="text-[11px] text-slate-400 flex items-center gap-1 overflow-x-auto py-0.5">
                <MapPin className="w-3 h-3 text-emerald-400 flex-shrink-0" />
                <span className="truncate">Jamshedpur • Kalinganagar • West Bokaro • Angul • Sukinda</span>
              </div>
            </div>
